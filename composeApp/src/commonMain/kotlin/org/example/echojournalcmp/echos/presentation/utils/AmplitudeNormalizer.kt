...
object AmplitudeNormalizer {
    ...
    // In AmplitudeNormalizer.kt
    private fun remapAmplitudes(amplitudes: List<Float>): List<Float> {
        val maxAmplitude = amplitudes.maxOrNull() ?: return emptyList()
        val inputRange = maxAmplitude - AMPLITUDE_MIN_OUTPUT_THRESHOLD
        val outputRange = MAX_OUTPUT - MIN_OUTPUT

        if (inputRange <= 0f) {
            // If all amplitudes are below the threshold, return a list of MIN_OUTPUT
            return amplitudes.map { MIN_OUTPUT }
        }
        ...
        return amplitudes.map { amplitude ->
            if (amplitude <= AMPLITUDE_MIN_OUTPUT_THRESHOLD) {
                ...
                val normalizedValue = (amplitude - AMPLITUDE_MIN_OUTPUT_THRESHOLD) / inputRange
                MIN_OUTPUT + (normalizedValue * outputRange)
            }
        }
    }
